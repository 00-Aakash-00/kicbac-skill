...
- Send only the opaque token value to merchant servers (`paymentToken` in JavaScript app JSON; `payment_token` at the gateway/Python SDK boundary). Never expose `security_key` in browser code.
